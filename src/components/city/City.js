...
import { CycleRaycast, BakeShadows, useCursor, OrbitControls } from '@react-three/drei'
  ...
  return (
    <div className={styles.container}>
      {/* <div className={styles.status}>
        {objects.map((_, i) => (<div key={i} className="dot" style={{ background: i === cycle ? '#70ffd0' : '#ccc' }} />))}
        {objects.length ? <div className="name" style={{ left: cycle * 14, padding: 2 }} children={objects[cycle].object.name} /> : null}
      </div> */}
      <Canvas shadows dpr={1.5} camera={{ position: [-10, 10, 5], fov: 70 }}>
        <OrbitControls makeDefault />
        <Stage />
        {/* {console.log(Array.from({ length: X }).map((_, i) => Array.from({ length: Y }, (_, i) => (
          <Building
            key={i}
            size={SIZE}
        ...
        {Array.from({ length: N }, (_, i) => (
          <Building
            key={i}
            height={i % 5 + ((Y - i) % 3) + SIZE}
            position={[(i - N / 2) % X, 0, (i - N / 2) / Y]}
          />
        ))}
...
function Building(props) {
  ...
  return (
    <mesh
      ...
      <boxGeometry args={[SIZE, props.height, SIZE]} />
